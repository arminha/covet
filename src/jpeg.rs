...
impl Jpeg {
    ...
    pub fn get_height_from_dnl(&self) -> Option<u16> {
        if let Some(dnl) = self.segments.iter().find(|s| s.marker() == DNL) {
...
fn split_segment(buffer: &mut Bytes) -> Result<Segment, String> {
    ...
    if buffer[0] != MARKER_START {
        return Err("segment not starting with 0xff".to_owned());
    }
    let marker = buffer[1];
    let len = match marker {
        SOI | EOI => 2usize,
        SOS => scan_for_sos_end(buffer.clone()),
        _ => {
...
mod test {
    ...
    fn split_segment_when_soi_then_create_segment() {
        let mut buffer = vec![MARKER_START, SOI].into();
        let segment = split_segment(&mut buffer).unwrap();
        assert_eq!(buffer.remaining(), 0);
        assert_eq!(segment.len(), 2);
        assert_eq!(segment.marker(), SOI);
    }

    #[test]
    fn split_segment_when_eoi_then_create_segment() {
        let mut buffer = vec![MARKER_START, EOI].into();
        let segment = split_segment(&mut buffer).unwrap();
        assert_eq!(buffer.remaining(), 0);
        assert_eq!(segment.len(), 2);
        assert_eq!(segment.marker(), EOI);
    }

    #[test]
    fn test2() {
        let buffer = load_image("scan_20220501_220231.jpeg");
    ...
    #[test]
    fn test() {
        let mut buffer = load_image("scan_20220501_220231.jpeg");
        ...
        assert_eq!(s1.marker(), SOI);
        assert_eq!(s2.marker(), APP0);
        assert_eq!(s2.len(), 18);
        assert_eq!(s3.marker(), DQT);
        assert_eq!(s3.len(), 69);
        assert_eq!(s4.marker(), DQT);
        assert_eq!(s4.len(), 69);
        assert_eq!(s5.marker(), SOF0);
        assert_eq!(s5.len(), 19);
        assert_eq!(s6.marker(), DHT);
        assert_eq!(s6.len(), 420);
        assert_eq!(s7.marker(), DRI);
        assert_eq!(s7.len(), 6);
        assert_eq!(s8.marker(), SOS);
        assert_eq!(s8.len(), 227569);
        assert_eq!(s9.marker(), DNL);
        assert_eq!(s9.len(), 6);
        assert_eq!(s10.marker(), EOI);
        assert_eq!(s10.len(), 2);
        assert!(s11.is_err());
    }
}
